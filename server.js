...
const storage = multer.diskStorage({
    ...
    filename: function (req, file, cb){
        cb(null, `${file.originalname.split('.')[0]}_${cuid()}${path.extname(file.originalname)}`)
    }
})
...
app.put('/upload', upload.single('file'), async (req, res)=>{
    ...
    if(!req?.body?.oldFileUrl){
        res.status(400).json({ msg: 'Você precisa enviar a oldFileUrl referente à URL do arquivo a ser substituído.'})
    }
    ...
    try {
        ...
        res.status(200).json({erro: true, fileUrl: fileUrl, msg: 'Houve um erro ao tentar excluir o arquivo anterior no servidor, mas tudo bem. O arquivo pode já ter sido excluído.'})
    }
})
...
app.delete('/upload', async (req, res)=>{
    ...
    try {
        ...
        res.status(500).json({msg: 'Houve um erro ao tentar o arquivo no servidor, mas tudo bem. O arquivo pode já ter sido excluído.'})
    }
})
